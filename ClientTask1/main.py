...
class Order:
    ...
    def create_template(self, vegetables, uom, clients):
        ...
        try:
            ...
            ws.cell(row=5, column=1, value="Sr. No.")
            ws.cell(row=5, column=2, value="Product")
            ws.cell(row=5, column=3, value="UOM")
            ws.cell(row=5, column=4, value="Qty.")

            for i in range(len(clients)):
                ws.cell(row=5, column=5+i, value=clients[i])
                    ...
                    for col_idx, cell_value in enumerate(ws2[5], start=1):  # Iterate over cells in the first row
                        if cell_value.value == row[0]:  # Check if the cell value matches the client identifier
                            if row[24]!=None:
                                ...
                                ws2.cell(row = 4, column = col_idx, value = initials_string.upper())
